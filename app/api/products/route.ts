  ...
  try {
    ...
    if (products.length === 0) {
      try {
        ...
        console.error('Database error seeding products:', dbErr)
        return NextResponse.json(
...
export async function POST(request: Request) {
  try {
    ...
    const finalPrice = isNaN(numPrice) ? 0 : Math.max(0, numPrice)
    ...
    try {
      ...
      console.error('Database error creating product:', dbErr)
      return NextResponse.json(
